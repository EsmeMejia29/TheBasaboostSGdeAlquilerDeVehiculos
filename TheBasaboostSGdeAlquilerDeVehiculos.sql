...
-- Ordenado para respetar las dependencias - Kate
...
ALTER TABLE EMPLEADO 
	ADD FOREIGN KEY(id_puesto) REFERENCES PUESTO_EMPLEADO(id_puesto);

ALTER TABLE EMPLEADO_DEPARTAMENTO 
	ADD FOREIGN KEY (id_empleado) REFERENCES EMPLEADO(id_empleado) ON DELETE CASCADE,
	ADD FOREIGN KEY (id_departamento) REFERENCES DEPARTAMENTO(id_departamento) ON DELETE CASCADE;

ALTER TABLE VEHICULOS 
	ADD FOREIGN KEY (id_marca) REFERENCES MARCA_VEHICULO(id_marca) ON DELETE CASCADE,
	ADD FOREIGN KEY (id_modelo) REFERENCES MODELO_VEHICULO(id_modelo) ON DELETE CASCADE,
	ADD FOREIGN KEY (id_tipo) REFERENCES TIPO_VEHICULO(id_tipo) ON DELETE CASCADE,
	ADD FOREIGN KEY (id_estado_v) REFERENCES ESTADO_VEHICULO(id_estado_v) ON DELETE CASCADE,
	ADD FOREIGN KEY (id_empleado) REFERENCES EMPLEADO(id_empleado) ON DELETE CASCADE;

ALTER TABLE MANTENIMIENTO 
	ADD FOREIGN KEY(id_empleado) REFERENCES EMPLEADO(id_empleado) ON DELETE CASCADE,
	ADD FOREIGN KEY(matricula) REFERENCES VEHICULOS(matricula) ON DELETE CASCADE;

ALTER TABLE RESERVA 
	ADD FOREIGN KEY (id_cliente) REFERENCES CLIENTE(id_cliente) ON DELETE CASCADE,
	ADD FOREIGN KEY (matricula) REFERENCES VEHICULOS(matricula) ON DELETE CASCADE,
	ADD FOREIGN KEY (id_estado_r) REFERENCES ESTADO_RESERVA(id_estado_r) ON DELETE CASCADE;

ALTER TABLE PAGO 
	ADD FOREIGN KEY (id_reserva) REFERENCES RESERVA(id_reserva) ON DELETE CASCADE,
	ADD FOREIGN KEY (id_metodo_pago) REFERENCES METODO_PAGO(id_metodo_pago) ON DELETE CASCADE;

ALTER TABLE SERVICIO_RESERVA 
	ADD FOREIGN KEY (id_reserva) REFERENCES RESERVA(id_reserva) ON DELETE CASCADE,
	ADD FOREIGN KEY (id_servicio_adicional) REFERENCES SERVICIOS_ADICIONALES(id_servicio_adicional) ON DELETE CASCADE;
